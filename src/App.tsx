...
const schema = z.object({
  ...
  MM: z.preprocess((val) => {
    ...
    return val;
  }, z.number({ invalid_type_error: "Can't be blank" }).min(1, "Invalid month").max(12, "Invalid month")),
  YY: z.preprocess((val) => {
    if (typeof val === "string" && val.trim() === "") return undefined;
    if (typeof val === "string") return Number(val);
    return val;
  }, z.number({ invalid_type_error: "Can't be blank" }).min(7, "Invalid year").max(99, "Invalid year")),
  CVC: z.preprocess((val) => {
    if (typeof val === "string" && val.trim() === "") return undefined;
    if (typeof val === "string") return Number(val);
    return val;
  }, z.number({ invalid_type_error: "Can't be blank" }).min(100, "Invalid CVC").max(999, "Invalid CVC")),
});
const reload = () => {
...
function App() {
  ...
  const mm =
    typeof mmValue === "number" && !isNaN(mmValue)
      ? mmValue.toString().padStart(2, "0")
      : "00";
  const yy =
    typeof yyValue === "number" && !isNaN(yyValue)
      ? yyValue.toString().padStart(2, "0")
      : "00";
    ...
    return (
      <div className="w-full h-screen flex bg-white">
        <div className="flex items-center justify-center flex-col w-4/12 h-full bg-bg-main-desktop bg-no-repeat bg-cover bg-center">
          <div className="w-full h-[800px] relative left-24 flex items-end justify-center flex-col gap-10">
            <div className="bg-bg-card-front bg-no-repeat bg-cover bg-center w-[447px] h-[245px] shadow-2xl rounded-lg p-5">
              ...
              <h1 className="mt-16 text-2xl text-white tracking-widest">
                0000 0000 0000 0000
              </h1>
        ...
        <div className="w-8/12 flex items-center justify-center flex-col">
          ...
          <h1 className="text-neutral-purple950 text-2xl font-semibold mb-2">
            THANK YOU!
          </h1>
  ...
  return (
    <div className="w-full h-screen flex bg-white">
      ...
      <div className="w-8/12 flex items-center justify-center">
        <form
          className="w-8/12 max-w-[445px]"
          onSubmit={handleSubmit(handleZustandSubmit)}
        >
          <div>
            <label
              htmlFor="CardholderName"
              className="block text-sm font-medium text-neutral-purple950 "
            >
              CARDHOLDER NAME
            </label>
            <input
              ...
              className={`w-full h-14 rounded-xl mt-3 border cursor-pointer ${
                errors.CardholderName ? "border-red-500" : "border-gray-300"
              } pl-5 focus:outline-none focus:border-purple-600`}
            />
            ...
            <div className="w-6/12">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className={`appearance-none w-full h-14 rounded-xl mt-3 border cursor-pointer ${
                    errors.MM ? "border-red-500" : "border-gray-300"
                  } pl-5 focus:outline-none focus:border-purple-600`}
                />
                <input
                  type="number"
                  ...
                  className={`appearance-none w-full h-14 rounded-xl mt-3 border cursor-pointer ${
                    errors.YY ? "border-red-500" : "border-gray-300"
                  } pl-5 focus:outline-none focus:border-purple-600`}
                />
              </div>
              {(errors.MM || errors.YY) && (
          ...
          <button
            type="submit"
            className="mt-8 bg-neutral-purple950 text-white w-full py-4 rounded-xl "
          >
            Confirm
          </button>
        </form>
